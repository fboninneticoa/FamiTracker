...
CPatternCompiler::CPatternCompiler(CFamiTrackerDoc* pDoc, unsigned int* pInstList, DPCM_List_t* pDPCMList,
                                   CCompilerLog* pLogger) :
	m_pDocument(pDoc),
		...
		*/
		...
		for (int j = 0; j < EffColumns; ++j)
		{
			...
			case EF_DUTY_CYCLE:
				...
				else if (ChanID != CHANID_TRIANGLE && ChanID != CHANID_DPCM)
				{
					// Not triangle and dpcm
