...
enum
{
	...
	// uint (0=default)
	CT_EXPANSION,
	// uint (0=none, 1=VRC6, 2=VRC7, 4=FDS, 8=MMC5, 16=N163, 32=S5B)
	CT_VIBRATO,
	// uint (0=old, 1=new)
	CT_SPLIT,
	// uint (32=default)
	...
	// macros
	CT_MACRO,
	// uint (type) uint (index) int (loop) int (release) int (setting) : int_list
	CT_MACROVRC6,
	// uint (type) uint (index) int (loop) int (release) int (setting) : int_list
	CT_MACRON163,
	// uint (type) uint (index) int (loop) int (release) int (setting) : int_list
	...
	// dpcm samples
	CT_DPCMDEF,
	// uint (index) uint (size) string (name)
	CT_DPCM,
	// : hex_list
	// instruments
	CT_INST2A03,
	// uint (index) int int int int int string (name)
	CT_INSTVRC6,
	// uint (index) int int int int int string (name)
	CT_INSTVRC7,
	// uint (index) int (patch) hex hex hex hex hex hex hex hex string (name)
	CT_INSTFDS,
	// uint (index) int (mod enable) int (m speed) int (m depth) int (m delay) string (name)
	CT_INSTN163,
	// uint (index) int int int int int uint (w size) uint (w pos) uint (w count) string (name)
	CT_INSTS5B,
	// uint (index) int int int int int  string (name)
	// instrument data
	CT_KEYDPCM,
	// uint (inst) uint (oct) uint (note) uint (sample) uint (pitch) uint (loop) uint (loop_point)
	CT_FDSWAVE,
	// uint (inst) : uint_list x 64
	CT_FDSMOD,
	// uint (inst) : uint_list x 32
	CT_FDSMACRO,
	// uint (inst) uint (type) int (loop) int (release) int (setting) : int_list
	CT_N163WAVE,
	// uint (inst) uint (wave) : uint_list
	// track info
	CT_TRACK,
	// uint (pat length) uint (speed) uint (tempo) string (name)
	CT_COLUMNS,
	// : uint_list (effect columns)
	CT_ORDER,
	// hex (frame) : hex_list
	// pattern data
	CT_PATTERN,
...
public:
	...
	bool ReadInt(int& i, int range_min, int range_max, CString* err)
	{
		...
		{
			if (err) err->Format(
				_T("Line %d column %d: expected integer in range [%d,%d], %d found."), line, c, range_min, range_max, i);
			return false;
		}
	...
	bool ReadHex(int& i, int range_min, int range_max, CString* err)
	{
		...
		{
			if (err) err->Format(
				_T("Line %d column %d: expected hexidecmal in range [%X,%X], %X found."), line, c, range_min, range_max, i);
			return false;
		}
		return true;
	}
	...
	CString& sResult)
{
	...
	else
	{
		...
		{
			sResult.Format(
				_T("Line %d column %d: note column should be 3 characters wide, '%s' found."), t.line, t.GetColumn(), sNote);
			return false;
		}
		...
		else
		{
			...
			{
			case TCHAR('c'): case TCHAR('C'): n = 0;
				break;
			case TCHAR('d'): case TCHAR('D'): n = 2;
				break;
			case TCHAR('e'): case TCHAR('E'): n = 4;
				break;
			case TCHAR('f'): case TCHAR('F'): n = 5;
				break;
			case TCHAR('g'): case TCHAR('G'): n = 7;
				break;
			case TCHAR('a'): case TCHAR('A'): n = 9;
				break;
			case TCHAR('b'): case TCHAR('B'): n = 11;
				break;
			default:
			...
			case TCHAR('#'): case TCHAR('+'): n += 1;
				break;
			case TCHAR('b'): case TCHAR('f'): n -= 1;
				break;
			default:
				sResult.Format(_T("Line %d column %d: unrecognized note '%s'."), t.line, t.GetColumn(), sNote);
				return false;
	...
	else
	{
		...
		{
			sResult.Format(
				_T("Line %d column %d: instrument column should be 2 characters wide, '%s' found."), t.line, t.GetColumn(), sInst);
			return false;
		}
		int h;
	...
	for (unsigned int e = 0; e <= pDoc->GetEffColumns(track, channel); ++e)
	{
		...
		if (sEff != _T("..."))
		{
			...
			{
				sResult.Format(
					_T("Line %d column %d: effect column should be 3 characters wide, '%s' found."), t.line, t.GetColumn(), sEff);
				return false;
			}
		...
		case CT_DPCM:
			{
				...
				while (!t.IsEOL())
				{
					...
					{
						sResult.Format(
							_T("Line %d column %d: DPCM sample %d overflow, increase size used in %s."), t.line, t.GetColumn(), dpcm_index,
							CT[CT_DPCMDEF]);
						return sResult;
					}
		...
		case CT_KEYDPCM:
			{
				...
				{
					sResult.Format(
						_T("Line %d column %d: instrument %d is not defined as a 2A03 instrument."), t.line, t.GetColumn(), i);
					return sResult;
				}
		...
		case CT_FDSWAVE:
			{
				...
				{
					sResult.Format(
						_T("Line %d column %d: instrument %d is not defined as an FDS instrument."), t.line, t.GetColumn(), i);
					return sResult;
				}
		...
		case CT_N163WAVE:
			{
				...
				{
					sResult.Format(
						_T("Line %d column %d: instrument %d is not defined as an N163 instrument."), t.line, t.GetColumn(), i);
					return sResult;
				}
