...
void CFamiTrackerDoc::OnFileSaveAs()
{
	...
	if (!AfxGetApp()->DoPromptFileName(newName, AFX_IDS_SAVEFILE, OFN_HIDEREADONLY | OFN_PATHMUSTEXIST, FALSE,
	                                   GetDocTemplate()))
		return;
...
BOOL CFamiTrackerDoc::SaveDocument(LPCTSTR lpszPathName) const
{
	...
	hOldFile = CreateFile(lpszPathName, FILE_READ_ATTRIBUTES, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
	                      NULL);
	...
	hOldFile = CreateFile(lpszPathName, FILE_WRITE_ATTRIBUTES, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
	                      NULL);
...
BOOL CFamiTrackerDoc::OpenDocumentNew(CDocumentFile& DocumentFile)
{
	...
	while (!DocumentFile.Finished() && !FileFinished && !ErrorFlag)
	{
		...
		else if (!strcmp(BlockID, FILE_BLOCK_SEQUENCES_VRC6))
		{
			...
			!strcmp(BlockID, FILE_BLOCK_SEQUENCES_N106))
		{
