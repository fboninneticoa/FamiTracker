                        ...
                        CWnd* pParentWnd, LPCTSTR lpszMenuName, DWORD dwExStyle, CCreateContext* pContext)
{
	...
	return CFrameWnd::Create(lpszClassName, lpszWindowName, dwStyle, newrect, pParentWnd, lpszMenuName, dwExStyle,
	                         pContext);
}
...
bool CMainFrame::CreateToolbars()
{
	...
	HBITMAP hbm = (HBITMAP)::LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_TOOLBAR_256), IMAGE_BITMAP, 0, 0,
	                                   LR_CREATEDIBSECTION);
...
bool CMainFrame::CreateDialogPanels()
{
	...
	if (!m_pFrameEditor->CreateEx(WS_EX_STATICEDGE, NULL, _T(""), WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL, rect,
	                              (CWnd*)&m_wndControlBar, 0))
	{
...
bool CMainFrame::CreateVisualizerWindow()
{
	...
	if (!m_pVisualizerWnd->CreateEx(WS_EX_STATICEDGE, NULL, _T("Samples"), WS_CHILD | WS_VISIBLE, rect,
	                                (CWnd*)&m_wndDialogBar, 0))
		return false;
...
bool CMainFrame::CreateInstrumentToolbar()
{
	...
	if (!m_wndInstToolBarWnd.CreateEx(0, NULL, _T(""), WS_CHILD | WS_VISIBLE, CRect(SX(288), SY(173), SX(472), SY(199)),
	                                  (CWnd*)&m_wndDialogBar, 0))
		return false;
	...
	if (!m_wndInstToolBar.CreateEx(&m_wndInstToolReBar, TBSTYLE_FLAT | TBSTYLE_TRANSPARENT,
	                               WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC) || !
		m_wndInstToolBar.LoadToolBar(IDT_INSTRUMENT))
		return false;

	m_wndInstToolBar.GetToolBarCtrl().SetExtendedStyle(TBSTYLE_EX_DRAWDDARROWS);

	// Set 24-bit icons
	HBITMAP hbm = (HBITMAP)::LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_TOOLBAR_INST_256), IMAGE_BITMAP, 0, 0,
	                                   LR_CREATEDIBSECTION);
...
void CMainFrame::OnBnClickedIncFrame()
{
	...
	CFrameAction* pAction = new CFrameAction(bChangeAll
		                                         ? CFrameAction::ACT_CHANGE_PATTERN_ALL
		                                         : CFrameAction::ACT_CHANGE_PATTERN);
